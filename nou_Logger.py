...
class Logger(object):
    ...
    def add_test_loss(self, loss_split, loss_merge, last=False):
        self.loss_test_aux.append([loss_split.data.cpu().numpy(), loss_merge.data.cpu().numpy()])
        if last:
            a = np.array(self.loss_test_aux)
            loss_split_test = a[:,0].mean()
            loss_merge_test = a[:,1].mean()
            self.loss_test.append([loss_split_test, loss_merge_test])
            self.loss_test_aux = []
